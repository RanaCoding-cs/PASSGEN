...
def menu_about():
    ...
    print(f"{GREEN}​ASCII Block Banner: A stylish and hacker-themed terminal interface with custom developer branding.{RESET}")
    print(f"{GREEN}​Smart Password Generation: Generates cryptographically secure passwords based on user-defined length and character types (Uppercase, Lowercase, Numbers, Special characters).{RESET}")
​    print(f"{GREEN}Save Option Prompt: A pop-up/prompt style feature allowing users to choose whether to save the password to a text file (passwords.txt) or not.{RESET}")
​    print(f"{GREEN}Password Manager (List View): Displays all saved passwords in a neat, numbered format.{RESET}")
​    print(f"{GREEN}Secure Total Count: Shows the total number of passwords saved without revealing them on screen for maximum privacy.{RESET}")
​    print(f"{GREEN}Indexed Removal System: Easily delete a specific password by entering its corresponding list number.{RESET}")
​    print(f"{GREEN}Cross-Platform: Works smoothly on Windows, Linux, and macOS (auto-clears terminal screen seamlessly).{RESET}")
